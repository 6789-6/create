...
export function LocalMemoryField({ node }: { node: RoseNode }) {
  ...
  const geometry = useMemo(() => createPointGeometry(3200, node.id.length * 1009, (_index, random) => {
  ...
  return (
    <group ref={ref} position={getNodePosition(node)} rotation={[0.08, -0.25, 0.04]}>
      <points geometry={geometry}><pointsMaterial size={0.038} transparent opacity={0.84} vertexColors depthWrite={false} blending={THREE.AdditiveBlending} /></points>
